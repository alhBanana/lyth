...
export default function Collection() {
  ...
  const isLinkedToActiveStory = useMemo(() => {
    ...
    return storyCollectionLinks.some(
      (link) =>
        link.collectionId === collection.id && link.storyId === story.id,
    );
  ...
  /**
   * Permanently deletes this Collection after user confirmation.
   */
  const handleDeleteCollection = async () => {
    if (!collection) return;

    const confirmed = window.confirm(
      `Delete "${collection.name}"?\n\nThis will permanently remove the Collection and unlink it from all Stories.`,
    );
    ...
    try {
      ...
      if (error instanceof ApiError) {
        ...
        setErrorMessage("Unable to delete Collection. Please try again.");
      }
    } finally {
      setIsSaving(false);
    }
    ...
    return (
      <div className="space-y-6">
        <SectionHeading
          ...
          subtitle="Choose a collection from the current story hub."
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <SectionHeading
        ...
        subtitle={collection.description || "Reusable collection"}
      />
        ...
        <Card>
          <div className="space-y-5">
            ...
            <p className="text-sm leading-7 text-slate-600">
              This collection is a focused workspace for{" "}
              {collection.name.toLowerCase()}, including practical notes and
              progress signals.
            </p>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[1.5rem] bg-[#F8F5EF] p-4 text-sm font-medium text-slate-950">
        ...
        <Card>
          ...
          <div className="mt-5 space-y-4">
            <button
              type="button"
              onClick={() =>
                navigate(`/library/collections/${collection.id}/edit`)
              }
              disabled={isSaving}
              className="w-full rounded-xl border border-[#2F5D50] px-4 py-2.5 text-sm font-semibold text-[#2F5D50] transition hover:bg-[#E9F1EE] disabled:cursor-not-allowed disabled:opacity-70"
            >
              Edit Collection
            </button>
            <button
            ...
            >
              {isLinkedToActiveStory
                ? `Unlink from ${story.title}`
                : `Link to ${story.title}`}
            </button>
            ...
            <button
              ...
              disabled={isSaving}
              className="w-full rounded-xl border border-[#C96B6B] px-4 py-2.5 text-sm font-semibold text-[#8A2E2E] transition hover:bg-[#FFF6F6] disabled:cursor-not-allowed disabled:opacity-70"
            >
              Delete Collection
            </button>
