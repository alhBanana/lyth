...
import {
  ...
  deleteCollection,
  fetchAppData,
  linkCollectionToStory,
  savePageNotes,
  savePageReflection,
  toggleTask,
  unlinkCollectionFromStory,
  updateCollection,
} from "../services/api";
...
export function StoryProvider({ children }: { children: ReactNode }) {
  ...
  const [storyCollectionLinks, setStoryCollectionLinks] = useState<
    StoryCollectionLinkEntry[]
  >([]);
  const [pages, setPages] = useState<PageEntry[]>(() => {
    const stored = migratePageData(
      parseStorage<PageEntry[]>(
        window.localStorage.getItem(STORAGE_KEYS.pages),
        initialPages,
      ),
    );
    if (
      !stored.some(
        (page) =>
          page.date === todayDateIdentifier && page.storyId === builtInStory.id,
      )
    ) {
    ...
    const hydrate = async () => {
      try {
        ...
        if (
          !hydratedPages.some(
            (page) =>
              page.date === todayDateIdentifier &&
              page.storyId === data.story.id,
          )
        ) {
          setPages([
            createPage(todayDateIdentifier, data.story.id),
            ...hydratedPages,
          ]);
        } else {
  ...
  const updatePageReflection = (pageId: EntityId, reflection: string) => {
    setPages((previous) =>
      previous.map((page) =>
        page.id === pageId ? { ...page, reflection } : page,
      ),
    );
  ...
  const addPageTask = (pageId: EntityId, title: string) => {
    ...
    setPages((previous) =>
      previous.map((page) =>
        page.id === pageId ? { ...page, tasks: [task, ...page.tasks] } : page,
      ),
    );
  ...
  const createNewCollection = async (input: {
    name: string;
    description: string;
    category?: string;
  ...
  const unlinkCollection = async (collectionId: string, storyId: string) => {
    ...
    setStoryCollectionLinks((previous) =>
      previous.filter(
        (item) =>
          !(item.storyId === storyId && item.collectionId === collectionId),
      ),
    );
  };

  /**
 * Updates an existing Library Collection and synchronises the
 * persisted result with the Collection state used by the UI.
 *
 * @param collectionId - The database ID of the Collection to update.
 * @param input - The editable Collection fields.
 * @returns The updated Collection returned by the API.
 */
const updateLibraryCollection = async (
   ...
   *
   * Any Story links associated with the Collection are also removed
   * from local state after a successful deletion.
   *
   * @param collectionId - The database ID of the Collection to delete.
   */
  const deleteLibraryCollection = async (collectionId: string) => {
    ...
    setStoryCollectionLinks((previous) =>
      previous.filter((link) => link.collectionId !== collectionId),
    );
  };
